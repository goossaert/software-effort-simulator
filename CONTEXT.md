...
**Constant Work CSV**:
An optional third CSV describing constant-work epics (see **Constant work**). Hand-authored to a *fixed* schema — `jira_key | epic_key`, `epic_name | building_block`, `key_result | KR | kr`, `moscow`, `team`, `quarter`, `tshirt_size | t_shirt_size` — read inline by `getConstantWorkEffort` (`index.html:1650-1662`) and `getConstantWorkEpics` (`index.html:1668-1686`); does *not* go through the **Column detector** family (no `detectXxxCol` invocation). Cached in the module-scoped `parsedConstantWork` (`index.html:1500`) with three observable states: `null` (no CSV ever loaded), `[]` (loaded but empty), `RowObject[]` (loaded). Rows are matched against the team-name column case-insensitively and against the quarter column exactly (trimmed). Unrecognised `tshirt_size` values silently contribute `0` PM via `tshirtToPersonMonths`'s miss branch — no warning, no error — see [ADR-0023](docs/adr/0023-constant-work-csv-deterministic-shift.md).
...
**Tab**:
One of four named views of a **Run**'s output: `Organization Level`, **Team Level tab**, **Team Projections tab**, **Initiatives tab**. The tab bar (`.tab-bar`, `index.html:982`) holds one `.tab-btn` per tab; exactly one carries the `.active` class at any time. After every Run, the active tab resets to `Organization Level` — see [ADR-0018](docs/adr/0018-tab-based-results-layout.md). The **Initiatives tab** ([ADR-0027](docs/adr/0027-editable-initiatives-tab-with-csv-export.md)) is the only tab that mutates simulator state on user interaction (via inline edit handlers on its dropdowns and numeric inputs); the other three are read-only.
...
**Initiatives tab**:
The fourth **Tab**, `#tab-initiatives` (`index.html:1027-1029`). Renders every row of `editedInitiatives` (`index.html:1497`, `1505`) as a wide horizontally-scrolling table with one column per CSV header (except the hidden columns `jira_link` and `emoji`, which are still serialised on export) and one `↓ Export CSV` toolbar button. Each cell is one of three kinds, decided per-render by `renderInitiativesTable` (`index.html:3136-3189`): (a) **non-editable plain text** for the detected **Initiative key** column and the detected name column (`nonEditable = new Set([detectedCols.initKeyCol, detectedCols.nameCol].filter(Boolean))`, `index.html:3144`) — protecting the **Initiative key**'s join contract with the **Epics CSV**; (b) **free-form `<input type="number" step="any">`** for the hardcoded `numericCols` set `{'added_value_impact', 'added_value_cost_saving'}` (`index.html:3148`) — those two value-impact fields are continuous-numeric in spirit, not categorical; (c) **`<select>` dropdown** of unique observed values for every other column, where the option list comes from `getUniqueColumnValues(parsedInitiatives)` (the *immutable* parsed array, so the pool stays stable across edits — see [ADR-0027](docs/adr/0027-editable-initiatives-tab-with-csv-export.md)) and always includes the row's current value at the head when absent from the unique set (`allOpts = opts.includes(current) ? opts : (current ? [current, ...opts] : opts)`, `index.html:3175`). Inline `onchange` (and `oninput` for numeric inputs) handlers write `this.value` — always a string — directly into `editedInitiatives[rowIdx][col]`. Edits are **commit-on-Run**: they land in `editedInitiatives` immediately but the other three tabs' charts and stats lag until the user presses **Run Simulation**. The tab is pre-rendered as part of every Run (in the run-button handler at `index.html:3362`, alongside `renderChart` / `renderTeamCharts` / `renderTeamProjections`); the table is rebuilt wholesale via a single `innerHTML` assignment. The `↓ Export CSV` button calls `exportInitiativesCSV` (`index.html:3191-3201`) which serialises `editedInitiatives` verbatim via `Papa.unparse` and downloads `initiatives-edited.csv` — the export is round-trip stable (re-uploading produces the same `detectedCols` and the same Run output as the original-plus-edits). Empty `editedInitiatives` renders the inline `No initiatives loaded.` message in place of the toolbar+table — see [ADR-0027](docs/adr/0027-editable-initiatives-tab-with-csv-export.md).
